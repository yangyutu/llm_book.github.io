...
The followings shows the performance on pretraining objectives. (Table from {cite:p}`lewis2019bart`).

```{table} Comparison of pre-training objectives. Performance varies considerably across tasks, but the BART models with text infilling demonstrate the most consistently strong performance. .
